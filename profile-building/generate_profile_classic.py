...
def assemble():
    ...
    profile = load_file("classic_profile")
    ...
    print("[+] Creating classic profile")
    with open(os.path.join(full_path, "profile-classic", "profile"), 'w') as f:
        f.write(profile)
    ...
    with open(os.path.join(full_path, "profile-classic", "tuya.device.active.json"), 'w') as f:
    ...
    with open(os.path.join(full_path, "profile-classic", "atop.online.debug.log.json"), 'w') as f:
        f.write('{"result": true, "t": 1644817605, "success": true}')
    with open(os.path.join(full_path, "profile-classic", "tuya.device.dynamic.config.ack.json"), 'w') as f:
        f.write('{"t": 1644817608, "success": true}')
    with open(os.path.join(full_path, "profile-classic", "tuya.device.dynamic.config.get.json"), 'w') as f:
    ...
    with open(os.path.join(full_path, "profile-classic", "tuya.device.timer.count.json"), 'w') as f:
        f.write('{"result": {"devId": "bf8d34cb20a1497360ivjw", "count": 0, "lastFetchTime": 0}, "t": 1644817612, "success": true}')
    with open(os.path.join(full_path, "profile-classic", "tuya.device.upgrade.silent.get.json"), 'w') as f:
        f.write('{"t": 1644817611, "success": true}')
    with open(os.path.join(full_path, "profile-classic", "tuya.device.uuid.pskkey.get"), 'w') as f:
...
if __name__ == '__main__':
    if not sys.argv[1:]:
        print('Usage: python generate_profile_classic.py <processed_directory>')
        sys.exit(1)
